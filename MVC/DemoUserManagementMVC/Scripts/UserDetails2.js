...
function validateEmailAndSubmitForm() {
    ...
    $.ajax({
        type: "GET",
        url: "/UserDetails2/CheckEmailExists?email=" + email + "&userID=" + userID,
...
function submitFormData() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.ajax({
                type: "POST",
                url: "/UserDetails2/SubmitFormData",
...
function PopulateStates(countryId, isCurrent, callback) {
    ...
    var stateUrl = isCurrent ? "/UserDetails2/GetStates?CountryID=" + countryId : "/UserDetails2/GetStates?CountryID=" + countryId;
...
$(document).ready(function () {
    ...
    if (userID != 0) {
        ...
        $.ajax({
            type: "GET",
            url: "/UserDetails2/GetFormDetails",
            data: { userId: userID },
            success: function (response) {
                console.log(response);
                var userDetails = response.UserDetails;
                var currentAddress = response.CurrentAddress;
                var permanentAddress = response.PermanentAddress;

                populateFields(userDetails);
                populateCurrentAddressFields(currentAddress);
                populatePermanentAddressFields(permanentAddress);
            },
            error: function (xhr, status, error) {
                console.log(error);
            }
